// Single source of truth cho nav links của Suối Mây — dùng chung Header + Footer.
export const NAV_LINKS = [
  { href: '#about', label: 'Về chúng tôi' },
  { href: '#services', label: 'Dịch vụ' },
  { href: '#menu', label: 'Bảng giá' },
  { href: '#gallery', label: 'Không gian' },
  { href: '#reviews', label: 'Cảm nhận' },
];

// CTA giữ chỗ — tách riêng vì dùng style khác trong Header/Footer.
export const BOOKING_LINK = { href: '#booking', label: 'Đặt lịch' };

// Social links — single source of truth cho thanh trên cùng (Header) + Footer.
// `path` là `d` của <path> trong viewBox 16×16.
...
// Email demo cho thanh liên hệ trên cùng (bookingSection chưa có field email).
