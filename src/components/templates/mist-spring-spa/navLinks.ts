...
//
// Every href is "#" today, and a delivered client site is not the place for
// four buttons that go nowhere. Both call sites render ACTIVE_SOCIAL_LINKS, so
// an icon appears only once it has somewhere to point; filling in an href here
// (or wiring these to the CMS later) is all it takes to bring them back.
...
/** Only the icons that lead somewhere. */
export const ACTIVE_SOCIAL_LINKS = SOCIAL_LINKS.filter((s) => s.href !== '#');

// Fallback for the top contact bar. bookingSection.email is the real source —
// an earlier note here claimed the field did not exist, and the header showed
// this address to every client as a result. It also read suoimay.spa@gmail.com,
// left over from the template's previous name.
export const CONTACT_EMAIL = 'hello@mistspring.vn';
